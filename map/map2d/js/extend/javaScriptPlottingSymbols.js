...
;
SuperMap.Geometry.LineString.createBezier1 = function(p, l, a) {
    if (a) {
        return SuperMap.Geometry.LineString.createBezier3(p, a)
    }
    var c = [];
    ...
    return new SuperMap.Geometry.LineString(c)
}
;
SuperMap.Geometry.LineString.calculatePointsFBZ2 = function(m, a) {
    if (!a) {
        a = 20
    }
    var c = [];
    ...
    for (var g = 0; g < m.length - 2; ) {
        ...
        for (var o = 0; o < 1; ) {
            ...
            var k = new SuperMap.Geometry.Point(j,h);
            c.push(k);
            o += e
        }
        g += 2;
...
;
SuperMap.Geometry.LineString.calculatePointsFBZ3 = function(o, a) {
    if (!a) {
        a = 20
    }
    var c = [];
    ...
    for (var g = 0; g < o.length - 3; ) {
        ...
        for (var q = 0; q < 1; ) {
            ...
            var m = new SuperMap.Geometry.Point(l,k);
            c.push(m);
            q += e
        }
        g += 3;
...
;
SuperMap.Geometry.LineString.calculatePointsFBZN = function(m, a) {
    if (!a) {
        a = m.length * 8
    }
    var c = [];
    ...
    for (var p = 0; p <= 1; ) {
        ...
        for (var g = 0; g < m.length; g++) {
            var h = SuperMap.Geometry.LineString.BEZ(d - 1, g, p);
            k += m[g].x * h;
            j += m[g].y * h
        }
        var l = new SuperMap.Geometry.Point(k,j);
        c.push(l);
        p += e
    }
    var f = c[c.length - 1];
...
;
SuperMap.Geometry.LineString.createBezier2 = function(c, a) {
    var d = SuperMap.Geometry.LineString.calculatePointsFBZ2(c, a);
    return new SuperMap.Geometry.LineString(d)
}
;
SuperMap.Geometry.LineString.createBezier3 = function(c, a) {
    var d = SuperMap.Geometry.LineString.calculatePointsFBZ3(c, a);
    return new SuperMap.Geometry.LineString(d)
}
;
...
;
SuperMap.Geometry.LineString.createBezierN = function(c, a) {
    var d = SuperMap.Geometry.LineString.calculatePointsFBZN(c, a);
    return new SuperMap.Geometry.LineString(d)
}
;
SuperMap.Geometry.LineString.BEZ = function(d, a, c) {
    return SuperMap.Geometry.LineString.combSort(d, a) * Math.pow(c, a) * Math.pow(1 - c, d - a)
}
;
SuperMap.Geometry.LineString.combSort = function(e, a) {
    var d = SuperMap.Geometry.LineString.factorial(e);
    var c = SuperMap.Geometry.LineString.factorial(a) * SuperMap.Geometry.LineString.factorial(e - a);
    return d / c
}
;
SuperMap.Geometry.LineString.factorial = function(d) {
    var a = 1;
    for (var c = 1; c <= d; c++) {
        a *= c
    }
...
;
SuperMap.Geometry.LineString.calculateCardinalPoints = function(A) {
    ...
    for (var z = 0; z <= w + 1 - 3; z++) {
        ...
        var f = B[z + 1];
        var d = B[z + 2];
        var o = new SuperMap.Geometry.Point();
        var h = new SuperMap.Geometry.Point();
        var G = new SuperMap.Geometry.Point(f.x - g.x,f.y - g.y);
        var a = new SuperMap.Geometry.Point(d.x - f.x,d.y - f.y);
        var v = Math.sqrt(G.x * G.x + G.y * G.y);
        var J = Math.sqrt(a.x * a.x + a.y * a.y);
        var u = new SuperMap.Geometry.Point(G.x / v,G.y / v);
        var m = new SuperMap.Geometry.Point(a.x / J,a.y / J);
        var H = new SuperMap.Geometry.Point(u.x + m.x,u.y + m.y);
        var c = Math.sqrt(H.x * H.x + H.y * H.y);
        var s = new SuperMap.Geometry.Point(H.x / c,H.y / c);
        ...
        l[z * 3 + 2 + 2] = h;
        if (z == 0) {
            var F = new SuperMap.Geometry.Point();
            var y = new SuperMap.Geometry.Point(o.x - g.x,o.y - g.y);
            var D = Math.sqrt(y.x * y.x + y.y * y.y);
            var j = new SuperMap.Geometry.Point(y.x / D,y.y / D);
            F.x = g.x + j.x * v * q * I;
            F.y = g.y + j.y * v * q * I;
            l[z * 3 + 0] = g;
            l[z * 3 + 1] = F
        ...
        if (z == w + 1 - 3) {
            var x = new SuperMap.Geometry.Point();
            var K = new SuperMap.Geometry.Point(h.x - d.x,h.y - d.y);
            var C = Math.sqrt(K.x * K.x + K.y * K.y);
            var r = new SuperMap.Geometry.Point(K.x / C,K.y / C);
            x.x = d.x + r.x * J * q * I;
            x.y = d.y + r.y * J * q * I;
            l[z * 3 + 2 + 3] = x;
            l[z * 3 + 2 + 4] = d
...
;
SuperMap.Geometry.LineString.createCloseCardinal = function(x) {
    ...
    for (var w = 0; w <= v - 1; w++) {
        ...
        var m = new SuperMap.Geometry.Point();
        var h = new SuperMap.Geometry.Point();
        var A = new SuperMap.Geometry.Point(f.x - g.x,f.y - g.y);
        var a = new SuperMap.Geometry.Point(d.x - f.x,d.y - f.y);
        var u = Math.sqrt(A.x * A.x + A.y * A.y);
        var D = Math.sqrt(a.x * a.x + a.y * a.y);
        var s = new SuperMap.Geometry.Point(A.x / u,A.y / u);
        var l = new SuperMap.Geometry.Point(a.x / D,a.y / D);
        var B = new SuperMap.Geometry.Point(s.x + l.x,s.y + l.y);
        var c = Math.sqrt(B.x * B.x + B.y * B.y);
        var r = new SuperMap.Geometry.Point(B.x / c,B.y / c);
...
;
SuperMap.Geometry.LineString.calculateCircle = function(A) {
    if (A.length < 3) {
        return A
    }
    var B = A.length
    ...
    if ((((a.x != g.x) && (a.y != g.y)) && (d.y == w * d.x + h)) || ((a.x == g.x) && (d.x == g.x)) || ((a.y == g.y) && (d.y == g.y)) || ((a.x == g.x) && (a.y == g.y)) || ((a.x == d.x) && (a.y == d.y)) || ((g.x == d.x) && (g.y == d.y))) {
        ...
        for (var z = 1; z < e; z++) {
            ...
            t.push(new SuperMap.Geometry.Point(o,n))
        }
        t.push(a)
    }
    return t
...
;
SuperMap.Geometry.LineString.createLineEPS = function(f) {
    var g = []
      , c = 0
      , a = f.length;
    if (f == null || a < 2) {
    ...
    for (var d = 0; d < a; ) {
        var e = f[d].type;
        if (e == "LTypeArc") {
            var h = SuperMap.Geometry.LineString.createLineArc(g, d, a, f);
            g = h[0];
            d = h[1]
        } else {
            g.push(f[d]);
...
,
SuperMap.Geometry.LineString.createLineArc = function(f, c, a, d) {
    if (c == 0) {
        var g = SuperMap.Geometry.LineString.addPointEPS(d, c, a, "LTypeArc");
        Array.prototype.push.apply(f, g[0]);
        c = g[1] + 1
    } else {
        if (c == a - 1) {
            var e = [d[c - 1], d[c]]
              , h = SuperMap.Geometry.LineString.calculateCircle(e);
...
,
SuperMap.Geometry.LineString.createLineCurve = function(f, c, a, d) {
    if (c == 0) {
        var g = SuperMap.Geometry.LineString.addPointEPS(d, c, a, "LTypeCurve");
        Array.prototype.push.apply(f, g[0]);
        c = g[1] + 1
    } else {
        if (c == a - 1) {
            var e = [d[c - 1], d[c]]
              , h = SuperMap.Geometry.LineString.calculatePointsFBZN(e);
            Array.prototype.push.apply(f, h);
            c++
        } else {
            var g = SuperMap.Geometry.LineString.addPointEPS(d, c, a, "LTypeCurve");
...
,
SuperMap.Geometry.LineString.addPointEPS = function(f, d, a, e) {
    var g = []
      , c = d + 1;
    if (d == 0) {
    ...
    if (e == "LTypeCurve") {
        var h = SuperMap.Geometry.LineString.calculatePointsFBZN(g)
    } else {
        if (e == "LTypeArc") {
            var h = SuperMap.Geometry.LineString.calculateCircle(g)
        }
    }
    return [h, c]
}
